...
public class Player : MonoBehaviour {
	...
	private enum JumpState{Start, Up, Down, Land};
	...
	{	
		...
		if((Input.GetAxis("Jump") != 0) && CharController.isGrounded)
		{
			...
			gameObject.transform.Find("SpritePlayer").SendMessage("JumpAnimation", (int) JumpState.Start);
			isJumping = true;
		}
		else 
		{
		...
		{
			gameObject.transform.Find("SpritePlayer").SendMessage("JumpAnimation", (int) JumpState.Land);
		}
		if(isJumping && CharController.velocity.y > 0)
		{
			gameObject.transform.Find("SpritePlayer").SendMessage("JumpAnimation", (int) JumpState.Up);
		}
		if(isJumping && CharController.velocity.y < 0)
		{
			gameObject.transform.Find("SpritePlayer").SendMessage("JumpAnimation", (int) JumpState.Down);
		}
		/*if(isJumping)
		{	dt++;
